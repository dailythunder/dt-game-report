...
LOG = logging.getLogger("dt_game_report.quarters_and_runs_analysis")

# These are only used for the optional CLI / fixtures mode.
...
def _load_summary(game_id: str) -> Dict[str, Any]:
    """Load an ESPN summary JSON from fixtures for CLI/testing use."""
...
def compute_quarter_team_and_player_totals(
    ...
    team_id_to_side: Dict[str, str],
    players_by_id: Dict[str, Dict[str, Any]],
) -> Tuple[Dict[int, Dict[str, Dict[str, int]]], Dict[int, Dict[str, Dict[str, Any]]]]:
...
def analyze_quarters_and_runs(
    ...
    if game_id is None:
        game_id = header.get("id") or comp.get("id")
...
def run_analysis_cli(game_id: Optional[str] = None) -> None:
    ...
    out = analyze_quarters_and_runs(summary, game_id=game_id)
...
def _main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Quarters/runs lab: compute quarter team/player totals + runs "
            "from ESPN summary JSON in fixtures."
        )
    )
    parser.add_argument(
        "--game-id",
