...
def patch_data(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Apply safe fixes on top of the JSON produced by fetch_espn_game:
    ...
    NOTE: This is *traditional stats only* on purpose.
    All advanced stat logic has been removed.
    """
    print("[DT Game Report] Patching data (traditional stats only)...")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate DT Game Report HTML from JSON (traditional stats only).")
    parser.add_argument(
        "--game-json",
        type=str,
        required=True,
